// ==UserScript==
// @name         Steam Buttons
// @namespace    SteamButtons@Teltta
// @version      2026-04-12
// @description  Add SteamHistory and Shadefall buttons to user profiles.
// @author       Teltta
...
(function() {
    ...
    function nameHistoryCB(steamID) {
        GM_xmlhttpRequest({
            ...
            onload: function (response) {
                try {
                    const shHTML = new DOMParser().parseFromString(response.responseText, "text/html");
                    ...
                    console.error("[SteamButtons] JSON parse error:", err);
                }
            },
            onerror: function (err) {
                console.error("[SteamButtons] GM request failed:", err);
            }
        });
    }
    ...
    function getSteamID()  {
        ...
        if (!steamID) {
            ...
            steamID = unsafeWindow?.g_rgProfileData?.steamid ?? null;
        }
